...
% SWE M-file for SWE.fig
%      SWE, by itself, creates a new SWE or raises the existing
%      singleton*.
%
%      H = SWE returns the handle to a new SWE or the handle to
%      the existing singleton*.
%
%      SWE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SWE.M with the given input arguments.
%
%      SWE('Property','Value',...) creates a new SWE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SWE_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SWE_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SWE

% Last Modified by GUIDE v2.5 22-Feb-2013 17:00:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SWE_OpeningFcn, ...
                   'gui_OutputFcn',  @SWE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SWE is made visible.
function SWE_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = SWE_OutputFcn(hObject, eventdata, handles) 
...
if (swkc+swcp11+swcp12+swcp21+swcp22+swcunit)==0
    disp('ERROR: No sweep variable is selected')
    return;
end
...
if (swr==0 || swstart<=0 || swstop <=0)
     ...
     disp('cannot be performed: please specify runs no. and set a non-zero kc value.')
     return
end
end
...
if nmis==1
    ...
    if swkc==1
        disp('...')
        disp('ERROR: kc-sweep and "no-mismatch" option cannot be selected together.');
        return;
    end
end
...
outsdnlcd=get(handles.outsdnlcd,'Value');
outsinlcd=get(handles.outsinlcd,'Value');
...
if (outINL || outDNL || outsdnlcd || outsinlcd ) 
  dnlv=zeros(swr,2^Nbit-2);
  inlv=zeros(swr,2^Nbit-2);
for i=1:swpoints
        disp(cat(2,'ITERATION ', num2str(i), ' OUT OF ' ,num2str(swpoints)))
        [ADNL,AINL,MDM,MDS,SDM,MIM,MIS,DSV,ISV,SIM] = STAT(sel,swr,Nbit,Vdd,Vss,xcunit(i),xkc(i),xmisos,cspec,xcbridge(i),xCpar11(i),xCpar12(i),xCpar21(i),xCpar22(i),STP,SBP,STBP,PEX11,PEX12,PEX21,PEX22,PEXB,SPEX);
        mxdnlv(i)=MDM;
        mxinlv(i)=MIM;
        sdnlcd(i)=SDM;
        sinlcd(i)=SIM;
   ...
   if outINL==1 
         ...
         ylabel('max|INL| [LSB]','Fontsize',8,'FontWeight','bold');
         grid on;  
    end
   if outDNL==1   
         figure()
         ...
         xlabel(xlab,'Fontsize',8,'FontWeight','bold');
         ylabel('max|DNL| [LSB]','Fontsize',8,'FontWeight','bold');
         grid on; 
   end
   if outsinlcd
         figure(3)
         ...
         plot(xplot,sinlcd,'Linewidth',1.5)
         title(cat(2,'Sweep Analysis Results: INL stdev vs ',xlab),'Fontsize',10,'FontWeight','bold');
         xlabel(xlab,'Fontsize',8,'FontWeight','bold');
   ...
   if outsdnlcd
         ...
         plot(xplot,sdnlcd,'Linewidth',1.5)
         title(cat(2,'Sweep Analysis Results: DNL stdev vs ',xlab),'Fontsize',10,'FontWeight','bold');
         xlabel(xlab,'Fontsize',8,'FontWeight','bold');
...
% --- Executes on selection change in popt.
function popt_Callback(hObject, eventdata, handles)
% hObject    handle to popt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns popt contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popt


% --- Executes during object creation, after setting all properties.
function popt_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in outsinlcd.
function outsinlcd_Callback(hObject, eventdata, handles)
% hObject    handle to outsinlcd (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of outsinlcd


% --- Executes on button press in outsdnlcd.
function outsdnlcd_Callback(hObject, eventdata, handles)
% hObject    handle to outsdnlcd (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of outsdnlcd
