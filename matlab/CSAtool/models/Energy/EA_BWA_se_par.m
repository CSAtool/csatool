function [NormEN,codici]=EA_BWA_se_par(Nbit,Vdd,Vss,cunit,kc,misos,cspec,cbridge,Cpar11,Cpar12,Cpar21,Cpar22,STP,SBP,STBP,PEX11,PEX12,PEX21,PEX22,PEXB)
...
for cod=(levs-levs):(levs-1)  %scorrono i livelli;
    ...
    Dout=fliplr(de2bi(cod,n));
      ...
      for i=1:n/2;
            ...
            if i==1
              Vta(k,j)=Vcm+FSR*(CA1*Dt1')/(CA1tot+Cx1+Cpar11);
              Vtb(k,j)=Vta(k,j)*(Cs/(Cs+CA2tot+Cpar12)); %per completezza
              ...
              En_ref=0;
               ...
               Vta(k,j)=Vcm+FSR*(CA1*Dt1')/(CA1tot+Cx1+Cpar11);
               Vtb(k,j)=Vta(k,j)*(Cs/(Cs+CA2tot+Cpar12)); %MA PER ORA NON IMPORTA: non ci sono ancora capacità switchate nel secondo array, per cui c'è solo l'attenuazione alla kirchoff;
